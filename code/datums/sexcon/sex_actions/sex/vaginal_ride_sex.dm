...
/datum/sex_action/vaginal_ride_sex/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	if(HAS_TRAIT(target, TRAIT_TINY) && !(HAS_TRAIT(user, TRAIT_TINY)))
		user.visible_message(span_warning("[user] заняв верхнюю позицию над [target], безуспешно пытается оседлать крохотный хер!"))
	else
		user.visible_message(span_warning("[user] занимает верхнюю позицию над [target] и направляет головку члена в свое лоно!"))
...
/datum/sex_action/vaginal_ride_sex/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	if(user.sexcon.do_message_signature("[type]"))
		if(HAS_TRAIT(target, TRAIT_TINY) && !(HAS_TRAIT(user, TRAIT_TINY)))
			user.visible_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()] скачает на стволе [target], безуспешно."))
			do_thrust_animate(user, target)
			return	//Return because male seelie cannot succesfully penetrate a large humen target
		user.visible_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()] скачет на [target]."))
	...
	if(target.sexcon.check_active_ejaculation())
		target.visible_message(span_lovebold("[target] наполняет лоно [user] своим горячим семенем!"))
...
/datum/sex_action/vaginal_ride_sex/on_finish(mob/living/carbon/human/user, mob/living/carbon/human/target)
	..()
	user.visible_message(span_warning("[user] поднимается с [target]."))
